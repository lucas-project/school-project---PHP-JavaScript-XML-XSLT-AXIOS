...
//                var row = table.getElementsByTagName("tr")[itemNo];
    ...
    if (oldQuantity > 0) {
        ...
        xHRObject.open("GET", "buyingCart.php?id=" + Number(new Date) + "&itemNumber=" + itemNo + "&action=add", true);
...
//                var row = table.getElementsByTagName("tr")[itemNo];
    ...
    xHRObject.open("GET", "buyingCart.php?id=" + Number(new Date) + "&itemNumber=" + itemNo + "&action=remove", true);
    xHRObject.onreadystatechange = function () {
        if (xHRObject.readyState == 4 && xHRObject.status == 200) {
            var response = xHRObject.responseText;
            document.getElementById('cart').innerHTML = response;
...
function confirmPurchase() {
    ...
    // -> amount to pay
    // clear shopping
    xHRObject.open("GET", "buyingCart.php?id=" + Number(new Date) + "&action=confirm", true);
    xHRObject.onreadystatechange = function () {
        if (xHRObject.readyState == 4 && xHRObject.status == 200) {
            var response = xHRObject.responseText;
            document.getElementById('cart').innerHTML = response;
...
function cancelPurchase() {
    ...
    xHRObject.open("GET", "buyingCart.php?id=" + Number(new Date) + "&action=cancel", true);
    xHRObject.onreadystatechange = function () {
        if (xHRObject.readyState == 4 && xHRObject.status == 200) {
            var response = xHRObject.responseText;
            document.getElementById('cart').innerHTML = response;
